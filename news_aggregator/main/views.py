...
def view_post(request, post_id):
    ...
    if request.method == "POST":
        params = {"id": post_id}
        response = requests.get("http://127.0.0.1:9999/postspost_by_id/", params=params)
        response_data = response.json()
...
def create_post(request):
    ...
    if request.method == "POST":
        ...
        try:
            _ = requests.post("http://127.0.0.1:9999/posts/create", data=data)
        except Exception:
...
def delete_post(request):
    ...
    if response_data:
        for item in response_data:
            data["posts"].append({"title": item.title})

    if request.method == "POST":
        post_id = request.POST.get("post_id")
        delete_data = {"id": post_id}
        _ = requests.post("http://127.0.0.1:9999/posts/delete", data=delete_data)
...
def update_post(request):
    ...
    if request.method == "POST":
        ...
        _ = requests.post("http://127.0.0.1:9999/posts/update", data=data)
